...
function StatusBadge({ status, method, paidAt, isManual, paidCents, expectedCents, awaitingValidation }: {
  ...
  if (isManual && status !== 'paid') return <span className="text-[10px] font-semibold px-2.5 py-1 rounded-full bg-[#F4F4F6] text-[#6B7280] font-[family-name:var(--font-nunito)] whitespace-nowrap">Hors app</span>
  if (status === 'paid') {
    ...
    return <span className="text-[10px] font-semibold px-2.5 py-1 rounded-full bg-[#E8F5EE] text-[#2A9D4E] font-[family-name:var(--font-nunito)] whitespace-nowrap">{parts.join(' — ')}</span>
  }
  return <span className="text-[10px] font-semibold px-2.5 py-1 rounded-full bg-[#FDF0EB] text-[#E8622A] font-[family-name:var(--font-nunito)] whitespace-nowrap">En attente</span>
}
    ...
    <div className="border-b border-[#F4F4F6] last:border-0">
      <div className="flex items-center gap-3 py-3.5 px-4">
        <button onClick={onClick} className="flex items-center gap-3 flex-1 min-w-0 text-left hover:opacity-80 transition-opacity">
          ...
          <p className="flex-1 text-sm font-semibold text-[#1A1F16] truncate font-[family-name:var(--font-nunito)]">{name}</p>
        ...
        {awaitingValidation && onValidate && (
          <button onClick={e => { e.stopPropagation(); onValidate() }}
            ...
            className="flex items-center gap-1 text-[10px] font-semibold text-[#2A9D4E] bg-[#E8F5EE] hover:bg-[#D1FAE5] px-2.5 py-1.5 rounded-lg transition-colors flex-shrink-0 font-[family-name:var(--font-nunito)]">
    ...
    <div className="space-y-4 pb-24">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-white rounded-xl border border-[#D1D1D6] p-3 text-center">
          <p className="text-xl font-[800] text-[#1A1F16] font-[family-name:var(--font-barlow)]">{totalCount}</p>
          <p className="text-[10px] text-[#6B7280] font-[family-name:var(--font-nunito)]">Membres</p>
        </div>
        <div className="bg-white rounded-xl border border-[#D1D1D6] p-3 text-center">
          <p className="text-xl font-[800] text-[#2A9D4E] font-[family-name:var(--font-barlow)]">{paidCount}</p>
        ...
        <div className="bg-white rounded-xl border border-[#D1D1D6] p-3 text-center">
          <p className="text-xl font-[800] text-[#2A9D4E] font-[family-name:var(--font-barlow)]">{(paidCents / 100).toFixed(0)} €</p>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {(Object.keys(SORT_LABELS) as SortKey[]).map(k => (
          <button key={k} onClick={() => setSort(k)}
            className={`text-xs font-semibold px-3 py-1 rounded-full transition-colors font-[family-name:var(--font-nunito)] ${sort === k ? 'bg-[#1A1F16] text-white' : 'bg-white border border-[#D1D1D6] text-[#6B7280]'}`}>
            {SORT_LABELS[k]}
          </button>
        ))}
      </div>
        ...
        return (
          <div key={cat} className="bg-white rounded-2xl border border-[#D1D1D6] shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#F4F4F6]">
              <p className="text-sm font-[800] text-[#1A1F16] font-[family-name:var(--font-barlow)] uppercase tracking-wide">{cat}</p>
              <span className="text-xs font-semibold text-[#6B7280] bg-[#F4F4F6] px-2.5 py-1 rounded-full font-[family-name:var(--font-nunito)]">{app.length + manual.length}</span>
            </div>
