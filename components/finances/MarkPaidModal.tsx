    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#F4F4F6]">
          <div>
            <p className="text-sm font-bold text-[#1A1F16] font-[family-name:var(--font-nunito)]">{memberName}</p>
            {canRemind && (
              <button onClick={() => void handleRemind()} disabled={reminding}
                className="flex items-center gap-1 mt-0.5 text-[11px] font-semibold text-[#6B7280] hover:text-[#E8622A] transition-colors font-[family-name:var(--font-nunito)] disabled:opacity-50">
        ...
        <div className="p-5 space-y-4">
          {isPaid ? (
            <>
              <div className="flex items-center gap-2 text-sm text-[#2A9D4E] bg-[#E8F5EE] rounded-xl px-4 py-3 font-[family-name:var(--font-nunito)]">
                <Check className="w-4 h-4" /> Déjà marqué comme payé
              </div>
              <button onClick={() => void handlePending()} disabled={saving}
                className="w-full flex items-center justify-center gap-2 h-10 border border-[#E8622A] text-[#E8622A] text-sm font-semibold rounded-xl hover:bg-[#FDF0EB] transition-colors disabled:opacity-50 font-[family-name:var(--font-nunito)]">
            ...
            <>
              ...
              <div>
                ...
                <div className="mt-1.5 flex items-center border border-[#D1D1D6] rounded-xl overflow-hidden bg-[#F4F4F6] h-11">
                  <input
                    type="number" min="0" step="0.01" value={amountStr}
                    onChange={e => setAmountStr(e.target.value)}
                    placeholder="0"
                    className="flex-1 h-full px-3 text-base font-bold bg-transparent focus:outline-none font-[family-name:var(--font-nunito)] text-[#1A1F16]"
                  />
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {METHODS.map(m => (
                    <button key={m.value} type="button" onClick={() => setMethod(m.value)}
                      className={`h-10 rounded-xl border text-sm font-semibold transition-colors font-[family-name:var(--font-nunito)] ${method === m.value ? 'border-[#2A9D4E] bg-[#E8F5EE] text-[#2A9D4E]' : 'border-[#D1D1D6] text-[#6B7280]'}`}>
                      {m.label}
                    </button>
                  ))}
                </div>
              ...
              {/* Info mode Stripe */}
              {isOnline && (
                <div className="bg-[#E8F5EE] border border-[#2A9D4E]/20 rounded-xl px-3 py-2.5 text-xs text-[#2A9D4E] font-semibold font-[family-name:var(--font-nunito)]">
              ...
              <button onClick={() => void handlePaid()} disabled={saving || amountCents <= 0}
                className={`w-full flex items-center justify-center gap-2 h-11 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-50 font-[family-name:var(--font-nunito)] ${isOnline ? 'bg-[#2A9D4E] hover:bg-[#238f44]' : isPartial ? 'bg-amber-500 hover:bg-amber-600' : 'bg-[#E8622A] hover:bg-[#d4571f]'}`}>
