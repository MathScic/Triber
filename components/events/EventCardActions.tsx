      ...
      <div className="space-y-2">
        ...
        {isMatch && canDelete && isLive && (
          <Link href={`/events/${event.id}/live`}
            ...
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-success text-white text-sm font-bold hover:bg-[#238742] transition-colors font-[family-name:var(--font-nunito)]">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {isMatch && canDelete && !isLive && (
            <Link href={`/events/${event.id}/live`}
              onClick={e => e.stopPropagation()}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary-light text-success text-xs font-bold hover:bg-[#d0ede0] transition-colors font-[family-name:var(--font-nunito)]">
          ...
          {isMatch && canDelete && (
            <button onClick={() => setShowLineup(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#E8E8EA] text-brand-dark text-xs font-semibold hover:bg-[#D1D1D6] transition-colors font-[family-name:var(--font-nunito)]">
              <Users className="w-3.5 h-3.5" /> Compo
            </button>
          )}
          <button onClick={onToggleAttendees}
            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors font-[family-name:var(--font-nunito)] ${showAttendees ? 'bg-brand-dark text-white' : 'bg-[#E8E8EA] text-brand-dark hover:bg-[#D1D1D6]'}`}>
            Présences <ChevronDown className={`w-3 h-3 ml-0.5 transition-transform ${showAttendees ? 'rotate-180' : ''}`} />
          </button>
          {isMatch && canDelete && (
            <button onClick={onToggleScore}
              className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors font-[family-name:var(--font-nunito)] ${showScore ? 'bg-brand-dark text-white' : 'bg-[#E8E8EA] text-brand-dark hover:bg-[#D1D1D6]'}`}>
