...
function PlayerRow({ player }: { player: Player }) {
  ...
  return (
    <div className="flex items-center gap-2 py-1">
      ...
      <span className="text-sm font-semibold text-brand-dark font-[family-name:var(--font-nunito)]">
        {display}
      </span>
    </div>
  )
...
export function LineupDisplay({ starters, subs }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#D1D1D6] shadow-sm p-4 space-y-4">
      ...
      {starters.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-success mb-1 font-[family-name:var(--font-nunito)] uppercase tracking-wide">
