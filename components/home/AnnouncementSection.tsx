      ...
      <div className="flex items-center justify-between">
        ...
        {canCreate && (
          <button onClick={() => setShowForm(v => !v)}
            className="text-xs font-semibold text-success hover:underline font-[family-name:var(--font-nunito)]">
            + Publier
          </button>
        )}
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-[#D1D1D6] p-4 space-y-3">
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Titre de l'annonce"
            className="w-full h-10 rounded-xl border border-[#D1D1D6] px-3 text-sm text-brand-dark focus:outline-none focus:ring-2 focus:ring-success" />
          <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Message…" rows={3}
            className="w-full rounded-xl border border-[#D1D1D6] px-3 py-2 text-sm text-brand-dark focus:outline-none focus:ring-2 focus:ring-success resize-none" />
          <div className="flex gap-2">
            <button onClick={handleCreate} disabled={saving || !title.trim() || !message.trim()}
              className="flex-1 h-10 rounded-xl bg-success text-white text-sm font-bold disabled:opacity-50 hover:bg-[#238742] transition-colors">
              {saving ? '…' : 'Publier'}
            </button>
